...
fight knight monster
  ...
  | otherwise = kG
  where
    kH = knightHealth knight
    kA = knightAttack knight
    kG = knightGold knight
    mH = monsterHealth monster
    mA = monsterAttack monster
    mG = monsterGold monster
...
--   maybe-treasure ;)
newtype Magic x = Magic x deriving (Show)

newtype Dragon x = Dragon (Magic x) deriving (Show)

newtype Treasure = Treasure String deriving (Show)
...
-- >>> revealArchEnemy (Nothing :: Maybe Bool)
-- "The arch-enemy of Nothing is NullPointerException"
...
daysToParty d
  ...
  | otherwise = diff
  where
    diff = fromEnum Friday - fromEnum d
...
takeDamage x attack = setHealth x (health - damage)
  where
    health :: Int
    health = getHealth x
    defense :: Int
    defense = getDefense x
    damage :: Int
    damage = if attack <= defense then 0 else attack - defense
...
instance Fighter Knight' where
  ...
  setHealth :: Knight' -> Int -> Knight'
  setHealth k h = k {knight'Health = h}

  getAttack :: Knight' -> Int
  getAttack = knight'Attack

  getDefense :: Knight' -> Int
  getDefense = knight'Defense

  getNextRound :: Knight' -> Knight'
  getNextRound k = k {knight'Actions = drop 1 (knight'Actions k)}
...
instance Fighter Monster' where
  ...
  setHealth :: Monster' -> Int -> Monster'
  setHealth m h = m {monster'Health = h}

  getAttack :: Monster' -> Int
  getAttack = monster'Attack

  getDefense :: Monster' -> Int
  getDefense _ = 0

  getNextRound :: Monster' -> Monster'
  getNextRound k = k {monster'Actions = drop 1 (monster'Actions k)}

class (Fighter a, Fighter b) => Fight a b where
  fightRound :: a -> b -> (a, b)

instance Fight Knight' Monster' where
  fightRound :: Knight' -> Monster' -> (Knight', Monster')
  fightRound = error ""

instance Fight Monster' Knight' where
  fightRound :: Monster' -> Knight' -> (Monster', Knight')
  fightRound = error ""

instance Fight Knight' Knight' where
  fightRound :: Knight' -> Knight' -> (Knight', Knight')
  fightRound = error ""

instance Fight Monster' Monster' where
  fightRound :: Monster' -> Monster' -> (Monster', Monster')
  fightRound = error ""

data Move = Fighter1 | Fighter2 deriving (Enum)

fight' :: (Fighter a, Fighter b, Fight a b) => a -> b -> Either a b
fight' x y = Left x'
  where
    (x', y') = fightRound x y
...
-- fight' = go Fighter1
--   where
--     go :: (Fighter a) => Move -> a -> a -> a
--     go Fighter1 f1 f2 =
--       go Fighter2 (getNextRound (actionFrom action f1)) (actionTo action f2)
--       where
--         action :: (Action a) => a
--         action = getAction f1
--     go Fighter2 f1 f2 = error ""
--     x :: (Fighter a) => a -> String
--     x = getName
...
-- fightResult = fight knight' monster'

-- fight' = error "Not implemented"

-- x = Knight' {actions = [], health = 0.5}
