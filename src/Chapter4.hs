...
deriving instance Show (f Int) => Show (IntBox f)
...
instance Applicative List where
  ...
  _ <*> Empty = Empty
  fs <*> (Cons x xs) = go (fs <*> xs) fs x
    where
      go :: List b -> List (a -> b) -> a -> List b
      go l Empty _ = l
      go l (Cons _f _fs) _x = Cons (_f _x) (go l _fs _x)

-- Test
foo :: Char -> Int
foo c = fromEnum c - fromEnum 'a'

listF :: List (Char -> Int)
listF = Cons foo (Cons (\c -> foo c + 1) (Cons (\c -> foo c + 2) Empty))

listX :: List Char
listX = Cons 'a' (Cons 'b' (Cons 'c' (Cons 'd' Empty)))

res :: List Int
res = listF <*> listX

-- >>> res
-- Cons 0 (Cons 1 (Cons 2 (Cons 1 (Cons 2 (Cons 3 (Cons 2 (Cons 3 (Cons 4 (Cons 3 (Cons 4 (Cons 5 Empty)))))))))))
...
instance Monad List where
  ...
  Cons x xs >>= f = conc (f x) (xs >>= f)
    where
      conc :: List b -> List b -> List b
      conc (Cons _x _xs) ys = Cons _x (conc _xs ys)
      conc Empty ys = ys
...
reverseBTree :: BTree a -> BTree a
reverseBTree Nil = Nil
reverseBTree (Node x y z) = Node x (reverseBTree z) (reverseBTree y)

inorderBTree :: BTree a -> [a]
inorderBTree Nil = []
inorderBTree (Node x y z) = inorderBTree y ++ [x] ++ inorderBTree z

preorderBTree :: BTree a -> [a]
preorderBTree Nil = []
preorderBTree (Node x y z) = [x] ++ preorderBTree y ++ preorderBTree z

postorderBTree :: BTree a -> [a]
postorderBTree Nil = []
postorderBTree (Node x y z) = postorderBTree y ++ postorderBTree z ++ [x]
